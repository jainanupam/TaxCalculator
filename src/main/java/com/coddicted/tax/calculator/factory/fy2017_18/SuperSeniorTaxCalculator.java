...
public class SuperSeniorTaxCalculator {
	...
	public static TaxInfo calculateTax(TaxCalculatorInputPOJO input){
		...
		if (netTaxableIncome.compareTo(BigDecimal.ZERO) > 0) {
			...
			// Slab1 differs for super senior citizens (> 80 years of age)
			BigDecimal taxableAmount = netTaxableIncome.compareTo(SLAB1_SUPER_SENIORS) > 0 ? SLAB1_SUPER_SENIORS
					: netTaxableIncome;
			taxAmount = taxableAmount.multiply(slab1Rate);
			// Re calculate the remaining net taxable income
			netTaxableIncome = netTaxableIncome.subtract(SLAB1_SUPER_SENIORS);
		} else {
